...
| v0.2.5-ignis0-store | Replace HashMap-backed `SubstanceStore` with the persistent hash trie spec (S-03) so `digest` is substitutive | **in progress** — `Store` trait + persistent `TrieStore` HAMT backend landed in `ignis0/src/store.rs` (Bagwell layout, collapse rule, substitutive digest proven by permutation/removal-order tests). Remaining: repoint `exec::Interp` / `fixed_point` / `Capability` from the concrete `SubstanceStore` to the trait (or `TrieStore`) and re-baseline the fixed-point digest |
