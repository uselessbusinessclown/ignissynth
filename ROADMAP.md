...
| v0.2.5-ignis0-store | Replace HashMap-backed `SubstanceStore` with the persistent hash trie spec (S-03) so `digest` is substitutive | ✓ done — `Store` trait + persistent `TrieStore` HAMT backend landed; interpreter, `fixed_point`, and `Capability` repointed from concrete `SubstanceStore` to `&mut dyn Store` so either backend can be used interchangeably |
