...
breakdown/   Synthesis tasks for implementer agents
kernel/      Reference seed core (placeholder until synthesis emits one)
```
...
2. `axioms/A0-first-principles.md` — the irreducible axioms. Everything
   downstream is required to be a theorem of these.
3. `axioms/A1-A8` — the eight derived axioms governing matter, energy,
   identity, causality, intent, attention, growth, and dissolution inside
   the habitat.
4. `synthesis/PROTOCOL.md` — how an implementing agent is required to
   *synthesize* (not write) any new component: from axioms → constraints
   → candidate forms → simulation → selection → commit.
5. `breakdown/00-SEED-TASKS.md` — the initial set of synthesis tasks.
